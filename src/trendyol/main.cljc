(ns trendyol.main
  "Kotodama WASM entrypoint for the Trendyol clean-room actor (L4) — Clojure port.
  ...
  facts (`trendyol.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "trendyol")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "trendyol_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "trendyol_cus"
    :fields [:email :firstName :lastName :phone]          :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "trendyol_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"trendyol.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "trendyol-compat" :tier tier :entities entities} 200])
...
(defn start! [] :trendyol-compat/ready)
